...
const LEGACY_TILE_CACHE_NAMES = ["map-tiles-v1"];
...
const APP_SHELL = [
  ...
  "./js/db/offlineTileRepository.js",
  // Services
...
async function tileStrategy(request) {
  ...
  try {
    ...
    return new Response("", { status: 204, statusText: "No cached tile" });
  }
}
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open(APP_CACHE_NAME).then((cache) => {
      console.info("[SW] precaching app shell", APP_CACHE_NAME);
  ...
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((key) => (
            (key.startsWith(CACHE_PREFIX) && ![APP_CACHE_NAME, ARCGIS_CACHE_NAME].includes(key)) ||
            LEGACY_TILE_CACHE_NAMES.includes(key)
          ))
          .map((key) => {
...
self.addEventListener("fetch", (event) => {
  ...
  if (event.request.mode === "navigate" && requestUrl.origin === self.location.origin) {
    event.respondWith(
      networkFirst(event.request, APP_CACHE_NAME).then((response) => {
        if (response.ok) {
          return response;
        }
