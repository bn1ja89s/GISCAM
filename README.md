...
El boton `Descargar zona visible` del panel de capas guarda tiles raster en Cache API (`map-tiles-v1`) y un manifiesto en `localStorage` (`exploracion-offline-tile-manifest-v1`). Al abrir la PWA sin conexion, si existen tiles descargados, el mapa arranca con ArcGIS y monta una capa `BaseTileLayer` local usando esos tiles. El fallback de grilla solo se usa si no hay tiles o si ArcGIS JS no puede iniciar.

Configuracion relacionada en `js/config.js`:

- `map.offlineTileLayerUrl`: servicio raster usado para descargar tiles si el WebMap usa un basemap vectorial.
- `map.offlineZoomMin` y `map.offlineZoomMax`: niveles de descarga por defecto.
- `arcgisSdk.useLocal`: cambia la carga del SDK a archivos locales.
- `arcgisSdk.localBasePath`: ruta esperada para self-host del SDK.

### Self-host opcional de ArcGIS JS

La app puede funcionar cacheando el CDN despues de una primera visita online, pero para una apertura offline mas robusta copia los assets del paquete `@arcgis/core` en:
...
Luego cambia en `js/config.js`:

```js
arcgisSdk: {
  useLocal: true,
  localBasePath: "../../vendor/arcgis/@arcgis/core",
}
```

Tambien debes publicar esa carpeta en GitHub Pages junto con el resto de archivos estaticos. No agregues API keys publicas en la configuracion.
