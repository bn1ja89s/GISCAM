...
export function renderTopbar({ appName, online, activeProject, pendingCount, canInstall, isSyncing }) {
  return `
    ...
    <div class="topbar__actions">
      ...
      <button class="button button--topbar" type="button" data-action="sync-now" ${isSyncing ? "disabled" : ""}>
        ${renderIcon("sync")}
        <span>${isSyncing ? "Sync..." : "Sincronizar"}</span>
      </button>
      <button class="ghost-button ghost-button--topbar ${canInstall ? "" : "hidden"}" type="button" data-action="install-app">
        ${renderIcon("install")}
        <span>Instalar</span>
      </button>
    </div>
