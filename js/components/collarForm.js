...
function renderPrimaryFieldSet(collar, currentTipo) {
  return `
    ...
    <label class="field">
      ...
      <input class="input" name="localizacion" value="${escapeHtml(collar.localizacion || "")}" placeholder="Sector, plataforma o descripcion corta">
    </label>
  `;
}

function renderSecondaryFieldSet(collar) {
  return `
    <div class="grid-fields">
      <label class="field">
      ...
      <label class="field">
        <span>Fecha</span>
        <input class="input" name="fecha" type="date" value="${escapeHtml(collar.fecha || todayValue())}" required>
      </label>
    </div>
  `;
}

function renderFieldSet(collar, currentTipo) {
  return `
    ${renderPrimaryFieldSet(collar, currentTipo)}
    ${renderSecondaryFieldSet(collar)}
  `;
}
    ...
    <form id="collar-form" class="stack collar-form ${isSheet ? "collar-form--sheet" : ""}" autocomplete="off">
      ...
      ${isSheet ? `
        <div class="capture-sheet__body">
          ...
          ${renderFieldSet(collar, currentTipo)}
