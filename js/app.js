...
import { adjustMapZoom, focusCollar, generateVisibleTilePackage, getCaptureEnabled, initMap, renderCollars, resetMapNorth, resolvePointElevation, setCaptureEnabled, setCurrentLocationPoint, setDraftPoint, startGpsNavigation, stopGpsNavigation } from "./services/mapService.js";
...
import { clearOfflineTiles, downloadTilePackage, formatBytes, getOfflineTileSummary, getStorageEstimate, hasOfflineTiles } from "./services/offlineTileService.js";
...
function buildMapFallbackMessage(error, online) {
  if (!online) {
    return hasOfflineTiles()
      ? "No fue posible iniciar ArcGIS con los tiles descargados. Se activara el mapa offline local para seguir capturando puntos por mapa."
      : "No hay mapa descargado para esta zona. Se activara el mapa offline local para seguir capturando puntos por mapa.";
  }
...
function buildArcGISCacheMessage() {
  ...
  if (!cacheMeta?.warmedAt) {
    return "Sin conexion. Se usaran tiles descargados si existen en este dispositivo.";
  }

  return `Sin conexion. Ultimo uso online del mapa: ${formatDateTime(cacheMeta.warmedAt)}. Las zonas no descargadas pueden no estar disponibles.`;
}

function buildOfflineMapBootstrapMessage() {
  if (hasOfflineTiles()) {
    return "No fue posible iniciar ArcGIS, pero existen tiles descargados. Revisa que el SDK ArcGIS este disponible offline o usa vendor/arcgis local.";
  }
  ...
  return "Descarga una zona visible online para habilitar el mapa offline real. Mientras tanto, se usara el mapa local de captura.";
}
...
function isArcGISMapEngine(state = getDerivedState()) {
  return state.mapEngine === "arcgis" || state.mapEngine === "arcgis-offline";
}
...
function refreshMapLayersPanel() {
  ...
  void refreshTileStorageEstimate();
}

async function refreshTileStorageEstimate() {
  const estimateEl = document.querySelector("#tile-storage-estimate");
  if (!estimateEl) {
    return;
  }

  const summary = getOfflineTileSummary();
  const estimate = await getStorageEstimate().catch(() => ({ usage: 0, quota: 0 }));
  const usedText = estimate.quota
    ? `${formatBytes(estimate.usage || 0)} / ${formatBytes(estimate.quota || 0)}`
    : formatBytes(estimate.usage || summary.byteSize || 0);
  const tileText = summary.tileCount === 1 ? "1 tile" : `${summary.tileCount} tiles`;

  estimateEl.textContent = summary.hasTiles
    ? `Mapa offline: ${tileText}. Cache estimada: ${formatBytes(summary.byteSize)}. Almacenamiento usado: ${usedText}.`
    : `Sin tiles descargados. Almacenamiento usado: ${usedText}.`;
}
...
async function mountMapPage() {
  ...
  const canUseDownloadedTiles = !state.online && hasOfflineTiles();
  ...
  if (state.online || canUseDownloadedTiles) {
    try {
      refreshMapSearchBar(true);
      await initMap(container, {
        onCapture: handleMapCapture,
        onSelectCollar: handleMapCollarSelection,
        offlineTiles: canUseDownloadedTiles,
      });
      mapLoaded = true;
      if (state.online) {
        rememberArcGISCache();
      ...
      store.setState({
        mapReady: true,
        mapEngine: canUseDownloadedTiles ? "arcgis-offline" : "arcgis",
        mapFallbackMessage: state.online ? "" : "Mapa offline listo. Algunas zonas no visitadas pueden no estar disponibles offline.",
      });
      refreshMapSearchBar();
      if (canUseDownloadedTiles) {
        showNotification("Mapa offline listo.", "success", 3600);
      }
    } catch (error) {
    ...
    await initOfflineMap(container, { onCapture: handleMapCapture, onSelectCollar: handleMapCollarSelection });
    mapLoaded = true;
    store.setState({ mapReady: true, mapEngine: "offline", mapFallbackMessage: "No hay mapa descargado para esta zona. " + buildOfflineMapBootstrapMessage() });
    refreshMapSearchBar(true);
  }
  ...
  if (!store.getState().online) {
    ...
    if ("serviceWorker" in navigator && "SyncManager" in window) {
      try {
        ...
        showNotification("Sin conexion. La sincronizacion se ejecutara automaticamente al recuperar internet.", "info", 4800);
      } catch {
        showNotification("Sin conexion. La sincronizacion se ejecutara cuando vuelvas a estar online.", "info", 4800);
      }
    } else {
      showNotification("Sin conexion. La sincronizacion se ejecutara cuando vuelvas a estar online.", "info", 4800);
    }
...
async function handleAction(action, source) {
  ...
  switch (action) {
    ...
    case "toggle-layers-panel":
      ...
      if (store.getState().layersPanelOpen) {
        await refreshTileStorageEstimate();
      }
    ...
    case "download-offline-tiles": {
      if (isOfflineMapActive()) {
        showNotification("La descarga de tiles solo esta disponible con el mapa ArcGIS activo.", "error", 4200);
        break;
      }

      if (!state.online) {
        showNotification("Necesitas conexion para descargar una zona offline nueva.", "error", 4200);
        break;
      }
      ...
      const minZoom = Number(appConfig.map.offlineZoomMin) || 10;
      const maxZoom = Number(appConfig.map.offlineZoomMax) || 16;
      const tilePackage = await generateVisibleTilePackage(minZoom, maxZoom);
      const tileTotal = tilePackage.tiles?.length || 0;
      if (!tileTotal) {
        showNotification("No se pudieron obtener las URLs de tiles del mapa actual. Asegurate de que el mapa ArcGIS este cargado.", "error", 4800);
        break;
      }
      ...
      if (progressEl) {
        ...
        progressEl.value = 0;
        progressEl.max = tileTotal;
      }

      if (statusEl) {
        statusEl.classList.remove("hidden");
        statusEl.textContent = `0 de ${tileTotal} tiles procesados`;
      }
      ...
      const result = await downloadTilePackage(tilePackage, {
        projectName: state.activeProject?.cod_exploracion || state.activeProject?.concesion_area || "",
        onProgress: ({ completed, total, downloaded, skipped, errors, bytes }) => {
          if (progressEl) {
            progressEl.value = completed;
          }

          if (statusEl) {
            statusEl.textContent = `${completed} de ${total} tiles procesados. Nuevos: ${downloaded}. Repetidos: ${skipped}. Errores: ${errors}. Tamano aprox: ${formatBytes(bytes)}.`;
          }
        },
      });
      ...
      showNotification(
        result.errors && result.downloaded === 0 && result.skipped === 0
          ? "No se pudo guardar la zona offline."
          : `Zona offline guardada correctamente. ${result.downloaded} nuevos, ${result.skipped} ya existian.${result.errors ? " Algunas zonas no visitadas pueden no estar disponibles offline." : ""}`,
        result.errors && result.downloaded === 0 && result.skipped === 0 ? "error" : "success",
        5200,
      );
    ...
    case "clear-tile-cache": {
      ...
      showNotification("Cache de tiles del mapa eliminado.", "success", 3200);
      await refreshTileStorageEstimate();
    ...
    case "start-draw-line": {
      ...
      if (!state.mapReady || !isArcGISMapEngine(state)) {
        throw new Error("El dibujo solo está disponible con el mapa ArcGIS activo.");
      }
      await startDrawingMode(drawMode, {
        onVertex: () => {
    ...
    case "cycle-line-unit": {
      const newUnit = cycleLineUnit();
      const m = getMeasurements();
      const label = formatLineDistance(m.distanceM || 0, newUnit);
      store.setState({ drawingMeasurement: label });
      const el = qs("#map-drawing-measurement");
      if (el) el.textContent = label;
      break;
    ...
    case "guide-to-guardado": {
      ...
      store.setState({ guardadoDetailId: "", guardadoExportModalId: "", pendingFocusGuardadoId: itemId });
      navigate("mapa");
      showNotification("Abriendo mapa con la ubicación de la foto.", "success", 3000);
      break;
    }
    ...
    case "query-guardado-elevation": {
      ...
      showNotification("Consultando elevación…", "info", 2500);
      ...
      if (elevated?.elevacion != null) {
        ...
        showNotification(`Elevación actualizada: ${Number(elevated.elevacion).toFixed(1)} m`, "success");
        renderCurrentPage();
      } else {
        showNotification("No se pudo obtener la elevación. Verifica tu conexión.", "error", 4000);
      }
      break;
    }
    ...
    case "export-guardado-item-geojson": {
      ...
      store.setState({ guardadoExportModalId: "" });
      renderCurrentPage();
      showNotification("GeoJSON exportado.", "success");
      break;
    }
    case "export-guardado-item-kml": {
      ...
      store.setState({ guardadoExportModalId: "" });
      renderCurrentPage();
      showNotification("KML exportado.", "success");
      break;
    }
    case "edit-guardado-area": {
      ...
      if (!store.getState().mapReady || !isArcGISMapEngine(store.getState())) {
    ...
    case "download-guardado-area-tiles":
      showNotification("Para descargar tiles de la zona, abre el mapa y usa el panel de capas → Descargar zona visible.", "info", 5000);
      break;
    case "export-guardado-geojson": {
      ...
      showNotification("GeoJSON exportado correctamente.", "success");
      break;
    }
    default:
      break;
...
async function handleDrawingSaveSubmit(form) {
  ...
  if (drawingState.mode === "area") {
    ...
    refreshMapDrawingSection();
    showNotification(isEditing ? `Área "${titulo}" actualizada.` : `Área "${titulo}" guardada.`, "success", 3500);
    ...
    const m = getMeasurements();
    const unidadDisplay = formatLineDistance(m.distanceM || 0);
    ...
    refreshMapDrawingSection();
    showNotification(`Línea "${titulo}" guardada.`, "success", 3500);
  }
}
...
async function bootstrap() {
  ...
  watchNetworkStatus(async (online) => {
    ...
    if (!online) {
      showNotification("Sin conexion — trabajando en modo offline.", "info", 4200);
    }
    ...
    if (previousState.route === "mapa") {
      if (!online && previousState.mapEngine === "arcgis" && previousState.mapReady) {
        ...
        showNotification(
          hasOfflineTiles()
            ? "Mapa offline listo."
            : "No hay mapa descargado para esta zona.",
          hasOfflineTiles() ? "success" : "info",
          4200,
        );
