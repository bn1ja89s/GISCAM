...
async function handleAction(action, source) {
  ...
  switch (action) {
    ...
    case "download-offline-tiles": {
      ...
      if (!view?.extent) {
        showNotification("No se pudo leer la zona visible del mapa actual.", "error", 4800);
        break;
      }
      ...
      try {
        ...
        showNotification(error.message || "No se pudo descargar el mapa offline.", "error", 5200);
        break;
      } finally {
        if (btnEl) {
      ...
      showNotification(
        summary?.blocked
          ? `El proveedor de tiles bloqueo parte de la descarga (${summary.blocked}). No se guardaron esos tiles.`
          : summary?.errors
            ? `Mapa offline descargado con avisos: ${summary.downloaded} nuevos, ${summary.skipped} existentes, ${summary.errors} errores.`
    ...
    case "clear-invalid-tiles": {
      ...
      showNotification(`Tiles invalidos eliminados: ${result.removed}. Restantes: ${result.remaining}.`, "success", 4200);
    ...
    case "clear-tile-cache": {
      ...
      showNotification("Mapa offline eliminado de IndexedDB.", "success", 3200);
  ...
  watchNetworkStatus(async (online) => {
    ...
    if (previousState.route === "mapa") {
      if (!online && previousState.mapEngine === "arcgis" && previousState.mapReady) {
        ...
        renderCurrentPage();
        showNotification("Sin conexion. Se activo el mapa offline local.", "info", 4200);
