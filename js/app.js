...
import { adjustMapZoom, focusCollar, getCaptureEnabled, getVisibleGeographicBounds, initMap, renderCollars, resetMapNorth, resolvePointElevation, setCaptureEnabled, setCurrentLocationPoint, setDraftPoint, startGpsNavigation, stopGpsNavigation } from "./services/mapService.js";
...
import { adjustOfflineMapZoom, focusOfflineCollar, initOfflineMap, reloadOfflineTiles, renderOfflineCollars, resetOfflineMapNorth, setOfflineCaptureEnabled, setOfflineCurrentLocationPoint, setOfflineDraftPoint } from "./services/offlineMapService.js";
import { buildTilePlan, clearOfflineTiles, downloadTilePlan, getOfflineTileSummary, hasOfflineTiles } from "./services/offlineTileService.js";
...
function buildMapFallbackMessage(error, online) {
  if (!online) {
    return "No fue posible iniciar ArcGIS sin conexion. Se usara el mapa offline local con tiles descargados si existen.";
  }
...
function buildOfflineMapBootstrapMessage() {
  return "Sin conexion. Si descargaste una zona, el mapa offline local cargara sus tiles; las zonas no visitadas pueden no estar disponibles.";
}
...
async function refreshOfflineTileStorageLabel() {
  ...
  const used = summary.storage?.usage ? formatBytes(summary.storage.usage) : "-";
  const quota = summary.storage?.quota ? formatBytes(summary.storage.quota) : "-";
  const offlineSize = summary.totalBytes ? `, tiles ${formatBytes(summary.totalBytes)}` : "";
  estimateEl.textContent = `Tiles guardados: ${summary.tileCount}${offlineSize}. Almacenamiento usado: ${used} / ${quota}`;
}
...
function refreshMapLayersPanel() {
  ...
  if (state.layersPanelOpen) {
    refreshOfflineTileStorageLabel().catch(() => {});
  }
}
...
async function mountMapPage() {
  ...
  const offlineTilesReady = !state.online && await hasOfflineTiles();
  const canTryCachedArcGIS = !state.online && hasArcGISCache() && !offlineTilesReady;
  ...
  if (offlineTilesReady) {
    ...
    store.setState({
      mapReady: true,
      mapEngine: "offline",
      mapFallbackMessage: "Mapa offline listo. Algunas zonas no visitadas pueden no estar disponibles offline.",
    });
    refreshMapSearchBar(true);
    showNotification("Mapa offline listo.", "success", 3600);
  } else if (state.online || canTryCachedArcGIS) {
    try {
      refreshMapSearchBar(true);
      ...
      refreshMapSearchBar(true);
      showNotification(state.online ? fallbackMessage : "No hay mapa descargado para esta zona.", state.online ? "error" : "info", 5200);
    }
  } else {
    refreshMapSearchBar(true);
    await initOfflineMap(container, { onCapture: handleMapCapture, onSelectCollar: handleMapCollarSelection });
    ...
    refreshMapSearchBar(true);
    showNotification("No hay mapa descargado para esta zona.", "info", 4200);
  }
...
async function handleAction(action, source) {
  ...
  switch (action) {
    ...
    case "download-offline-tiles": {
      ...
      const bounds = getVisibleGeographicBounds();
      if (!bounds) {
        showNotification("No se pudo obtener la zona visible actual. Espera a que el mapa termine de cargar.", "error", 4800);
        break;
      }

      const plan = buildTilePlan(bounds, {
        minZoom: appConfig.map.offlineTileMinZoom,
        maxZoom: appConfig.map.offlineTileMaxZoom,
        projectUuid: state.activeProject?.uuid || "",
        projectName: state.activeProject?.cod_exploracion || state.activeProject?.concesion_area || "",
      });

      await refreshOfflineTileStorageLabel();

      if (progressEl) {
        progressEl.classList.remove("hidden");
        progressEl.value = 0;
        progressEl.max = plan.tiles.length;
      }

      if (statusEl) {
        statusEl.classList.remove("hidden");
        statusEl.textContent = `0 de ${plan.tiles.length} tiles descargados`;
      }
      ...
      const result = await downloadTilePlan(plan, {
        onProgress: (progress) => {
          if (progressEl) {
            progressEl.value = progress.completed;
          }

          if (statusEl) {
            statusEl.textContent = `${progress.completed} de ${progress.total} procesados, ${progress.downloaded} nuevos, ${progress.skipped} repetidos${progress.errors ? `, ${progress.errors} errores` : ""}. ${formatBytes(progress.totalBytes)}`;
          }
        },
      });
      ...
      showNotification(
        result.errors && result.manifest.tile_count === 0
          ? "No se pudo guardar ningun tile de la zona visible."
          : `Zona offline guardada correctamente: ${result.manifest.tile_count} tiles (${formatBytes(result.totalBytes)}).${result.errors ? " Algunas zonas no visitadas pueden no estar disponibles offline." : ""}`,
        result.errors && result.manifest.tile_count === 0 ? "error" : "success",
        5200,
      );
    ...
    case "clear-tile-cache": {
      ...
      await refreshOfflineTileStorageLabel();
      await reloadOfflineTiles();
      showNotification("Cache offline del mapa eliminado.", "success", 3200);
    ...
    case "start-draw-line": {
      ...
      if (!state.mapReady || state.mapEngine !== "arcgis") {
  ...
  watchNetworkStatus(async (online) => {
    ...
    if (previousState.route === "mapa") {
      if (!online && previousState.mapEngine === "arcgis" && previousState.mapReady) {
        if (await hasOfflineTiles()) {
          store.setState({ mapEngine: "offline", mapFallbackMessage: "Mapa offline listo. Algunas zonas no visitadas pueden no estar disponibles offline." });
          renderCurrentPage();
          showNotification("Mapa offline listo.", "success", 4200);
        } else {
          store.setState({ mapFallbackMessage: "No hay mapa descargado para esta zona." });
          refreshMapToolbarSection();
          refreshMapStatusChip();
          refreshMapProjectBanner();
          showNotification("No hay mapa descargado para esta zona.", "info", 4200);
