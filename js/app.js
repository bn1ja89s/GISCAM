...
async function handleAction(action, source) {
  ...
  switch (action) {
    ...
    case "download-offline-tiles": {
      ...
      showNotification(
        summary?.blocked
          ? summary.lastErrorMessage || "API key invalida, restringida o dominio no permitido."
          : summary?.errors
            ? summary.lastErrorMessage || "No se pudo descargar el tile. Revisa la consola para ver el status real de MapTiler."
