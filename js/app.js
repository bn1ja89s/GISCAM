...
import { clone, createUuid, enrichPointForProject, formatDateTime, generarIdDesdeTecnicoYCollar, nowIso, toNullableNumber } from "./core/helpers.js";
...
import { createLaboratorio, deleteLaboratorio, obtenerLaboratoriosDeArcGIS, obtenerLaboratoriosLocales, updateLaboratorio } from "./services/laboratorioService.js";
...
import { cerrarSesion, initAuth, login, registrar, traducirErrorFirebase } from "./services/firebaseService.js";
...
async function cargarLaboratorios({ render = true } = {}) {
  try {
    const assays = store.getState().assays;
    const deSurvey = await obtenerLaboratoriosDeArcGIS(assays);
    const locales = await obtenerLaboratoriosLocales();
    const mapa = new Map();

    [...locales, ...deSurvey].forEach((laboratorio) => {
      const key = laboratorio.uuid || laboratorio.id || laboratorio.id_local;
      if (key) {
        mapa.set(key, laboratorio);
      }
    });

    const laboratorios = Array.from(mapa.values())
...
async function handleCollarSubmit(form) {
  ...
  if (!data.uuid && savedCollar?.uuid) {
    const flowSampleId = generarIdDesdeTecnicoYCollar(state.activeProject?.tecnico || "", savedCollar.hole_id || data.hole_id || "");
    store.setState({
      captureFlow: {
        active: true,
        step: "survey",
...
function activarListenersAuth() {
  ...
  document.getElementById("btn-login")?.addEventListener("click", async () => {
    ...
    try {
      ...
      errorEl.textContent = traducirErrorFirebase(error.code);
