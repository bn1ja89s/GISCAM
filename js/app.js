...
import { adjustMapZoom, focusCollar, generateTileUrls, getCaptureEnabled, initMap, renderCollars, resetMapNorth, resolvePointElevation, setCaptureEnabled, setCurrentLocationPoint, setDraftPoint, startGpsNavigation, stopGpsNavigation } from "./services/mapService.js";
...
async function handleAction(action, source) {
  ...
  switch (action) {
    ...
    case "download-offline-tiles": {
      ...
      const tileUrls = await generateTileUrls(10, 16);
      if (!tileUrls.length) {
      ...
      if (progressEl) {
        ...
        progressEl.value = 0;
        progressEl.max = tileUrls.length;
      }

      if (statusEl) {
        statusEl.classList.remove("hidden");
        statusEl.textContent = `0 de ${tileUrls.length} tiles descargados`;
      }
      ...
      for (const url of tileUrls) {
        ...
        if (progressEl) {
          progressEl.value = downloaded + errors;
        }

        if (statusEl) {
          statusEl.textContent = `${downloaded} de ${tileUrls.length} tiles descargados${errors ? ` (${errors} errores)` : ""}`;
        }
      }
      ...
      showNotification(
        `Descarga completada: ${downloaded} tiles guardados.${errors ? ` ${errors} tiles no disponibles.` : ""}`,
        errors && downloaded === 0 ? "error" : "success",
        5200,
      );
