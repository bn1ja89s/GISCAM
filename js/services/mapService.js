...
function resolveAppUrl(url) {
  ...
  return new URL(value.replace(/^\.\//, ""), document.baseURI).href.replace(/\/+$/, "");
}

function getArcGISBaseUrl() {
  return resolveAppUrl(appConfig.arcgis?.baseUrl || "https://js.arcgis.com/4.33/@arcgis/core");
}

function getArcGISAssetsPath() {
  return resolveAppUrl(appConfig.arcgis?.assetsPath || `${getArcGISBaseUrl()}/assets`);
}

function getArcGISCssUrl() {
  return resolveAppUrl(appConfig.arcgis?.cssUrl || `${getArcGISAssetsPath()}/esri/themes/light/main.css`);
}

function ensureArcGISCss() {
  const cssUrl = getArcGISCssUrl();
  let link = document.querySelector("link[data-arcgis-css]");
  if (link?.href === cssUrl) {
    return;
  }
...
export function getVisibleGeographicBounds() {
  const view = mapState.view;
  const { webMercatorUtils } = getArcGISModules() || {};
  if (!view?.extent) {
    return null;
  }

  try {
    const extent = view.extent.spatialReference?.isWebMercator && webMercatorUtils
      ? webMercatorUtils.webMercatorToGeographic(view.extent)
      : view.extent;

    return {
      xmin: roundNumber(Number(extent.xmin), 6),
      ymin: roundNumber(Number(extent.ymin), 6),
      xmax: roundNumber(Number(extent.xmax), 6),
      ymax: roundNumber(Number(extent.ymax), 6),
    };
  } catch {
    return null;
  }
