...
import { buildOfflineTileUrl, createTileLookup, getLatestOfflineTileManifest, getTileBounds } from "./offlineTileService.js";
...
const offlineMapState = {
  ...
  tileManifest: null,
  tileLookup: new Set(),
  tileZoom: appConfig.map?.offlineTileMaxZoom || 16,
};
...
function getDownloadedTileExtent() {
  const bounds = offlineMapState.tileManifest?.bounds;
  if (
    Number.isFinite(Number(bounds?.xmin)) &&
    Number.isFinite(Number(bounds?.ymin)) &&
    Number.isFinite(Number(bounds?.xmax)) &&
    Number.isFinite(Number(bounds?.ymax))
  ) {
    return normalizeExtent(bounds);
  }
...
function buildExtentFromPoints(points) {
  if (!points.length) {
    return getDownloadedTileExtent() || getDefaultExtent();
  }
...
function getVisibleTilesForZoom(zoom) {
  const manifest = offlineMapState.tileManifest;
  if (!manifest?.tiles?.length || !offlineMapState.extent) {
    return [];
  }

  return manifest.tiles
    .filter((tile) => tile.z === zoom && offlineMapState.tileLookup.has(`${tile.z}/${tile.x}/${tile.y}`))
    .map((tile) => ({
      ...tile,
      bounds: getTileBounds(tile.z, tile.x, tile.y),
    }))
    .filter((tile) => extentsIntersect(offlineMapState.extent, tile.bounds));
}

function chooseTileZoom() {
  const manifest = offlineMapState.tileManifest;
  if (!manifest?.tiles?.length) {
    return offlineMapState.tileZoom;
  }

  const minZoom = Number(manifest.min_zoom || appConfig.map?.offlineTileMinZoom || 10);
  const maxZoom = Number(manifest.max_zoom || appConfig.map?.offlineTileMaxZoom || 16);
  const preferredZoom = Math.min(Math.max(Number(offlineMapState.tileZoom) || maxZoom, minZoom), maxZoom);

  for (let zoom = preferredZoom; zoom >= minZoom; zoom -= 1) {
    if (getVisibleTilesForZoom(zoom).length) {
      return zoom;
    }
  }
...
function renderOfflineTiles() {
  if (!offlineMapState.tileManifest?.tiles?.length) {
    return "";
  }

  const zoom = chooseTileZoom();
  const tiles = getVisibleTilesForZoom(zoom);
  if (!tiles.length) {
    return "";
  }

  return tiles.map((tile) => {
    const topLeft = projectToScreen({ longitude: tile.bounds.xmin, latitude: tile.bounds.ymax });
    const bottomRight = projectToScreen({ longitude: tile.bounds.xmax, latitude: tile.bounds.ymin });
    ...
    const url = tile.url || buildOfflineTileUrl(tile.z, tile.x, tile.y, offlineMapState.tileManifest.tile_layer_url);

    return `<image class="offline-map-tile" href="${escapeAttribute(url)}" x="${x}" y="${y}" width="${width}" height="${height}" preserveAspectRatio="none" />`;
  }).join("");
}
...
function zoomExtent(factor) {
  ...
  if (offlineMapState.tileManifest?.tiles?.length) {
    const minZoom = Number(offlineMapState.tileManifest.min_zoom || appConfig.map?.offlineTileMinZoom || 10);
    const maxZoom = Number(offlineMapState.tileManifest.max_zoom || appConfig.map?.offlineTileMaxZoom || 16);
    const delta = factor < 1 ? 1 : -1;
    offlineMapState.tileZoom = Math.min(Math.max(offlineMapState.tileZoom + delta, minZoom), maxZoom);
  }
}
...
async function loadOfflineTileManifest() {
  const manifest = await getLatestOfflineTileManifest().catch(() => null);
  offlineMapState.tileManifest = manifest;
  offlineMapState.tileLookup = createTileLookup(manifest);

  if (manifest?.max_zoom) {
    offlineMapState.tileZoom = Number(manifest.max_zoom);
  }

  if (!offlineMapState.userAdjustedExtent && manifest?.bounds) {
    offlineMapState.extent = normalizeExtent(manifest.bounds);
  }
}
...
export async function reloadOfflineTiles() {
  await loadOfflineTileManifest();
  maybeFitExtent(true);
  renderOfflineMap();
}
