...
const TILE_ERROR_MESSAGE = "No se pudo descargar el tile. Revisa tu API key o proveedor de mapas.";
const BLOCKED_TILE_STATUSES = [403, 404, 429, 500];
...
function getTileDownloadErrorMessage({ status = 0, contentType = "" } = {}) {
  if (status === 401 || status === 403) {
    return "API key invalida, restringida o dominio no permitido.";
  }

  if (status === 404) {
    return "URL de tile mal formada o mapa no existe.";
  }

  if (status === 429) {
    return "Limite de solicitudes de MapTiler alcanzado.";
  }

  if (contentType && !isValidTileContentType(contentType)) {
    return "Respuesta invalida: el content-type del tile no es image/.";
  }

  return TILE_ERROR_MESSAGE;
}

function logTileDownloadFailure({ response = null, contentType = "", url = "", reason = "" } = {}) {
  console.warn("[offline-map] tile download failed", {
    reason,
...
export async function downloadVisibleArea(view, options = {}) {
  ...
  for (const tile of tiles) {
    ...
    const url = tile.url;
    if (processed === 1) {
      console.log("[offline-map] test tile url:", url);
    }
