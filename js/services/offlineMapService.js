...
const VALID_TILE_CONTENT_TYPES = ["image/png", "image/jpeg", "image/jpg", "image/webp"];
...
function isTileProviderConfigured() {
  return !String(TILE_CONFIG.urlTemplate || "").includes("TU_PROVEEDOR");
}

function isValidTileContentType(contentType = "") {
  const normalized = String(contentType).split(";")[0].trim().toLowerCase();
  return VALID_TILE_CONTENT_TYPES.includes(normalized);
}
...
export async function downloadVisibleArea(view, options = {}) {
  ...
  for (const tile of tiles) {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn("[offline-map] invalid tile response", response.status, contentType, tile.url);
      ...
      if (!blob.size || !isValidTileContentType(blob.type || contentType)) {
        console.warn("[offline-map] invalid tile blob", response.status, blob.type || contentType, tile.url);
        invalid++;
        errors++;
        options.onProgress?.({ processed, downloaded, skipped, errors, blocked, invalid, total: tiles.length, tile });
        continue;
...
export async function getOfflineMapDiagnostics({ mapEngine = "unknown", online = navigator.onLine } = {}) {
  ...
  return {
    ...
    tileUrlTemplate: TILE_CONFIG.urlTemplate,
