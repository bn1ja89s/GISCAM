...
const TILE_ERROR_MESSAGE = "No se pudo conectar con MapTiler desde este origin. Revisa la consola para ver el fallo de red.";
const UNAUTHORIZED_ORIGIN_MESSAGE = "Este dominio no está autorizado en MapTiler. Usa localhost o GitHub Pages.";
...
function getCurrentOriginAuthorization(origin = location.origin) {
  ...
  return {
    ...
    authorizedOrigins: AUTHORIZED_TILE_ORIGINS,
    note: authorized
      ? "Origin autorizado para la API key de MapTiler."
      : "Origin no autorizado: usa localhost, 127.0.0.1 o https://bn1ja89s.github.io/GISCAM/.",
  };
}

function getTileDownloadErrorMessage({ status = 0, contentType = "" } = {}) {
  if (status === 401) {
    return "MapTiler rechazó la API key. Revisa si la key es correcta.";
  }

  if (status === 403) {
    return "MapTiler bloqueó el dominio actual. Abre la app desde localhost o desde https://bn1ja89s.github.io/GISCAM/.";
  }

  if (status === 404) {
    return "URL de tile mal formada. Revisa TILE_CONFIG.urlTemplate.";
  }

  if (status === 429) {
    return "Límite gratuito de MapTiler alcanzado.";
  }

  if (contentType && !isValidTileContentType(contentType)) {
    return "MapTiler devolvió una respuesta que no es imagen.";
  }

  return status
    ? `MapTiler devolvió status ${status}. Revisa la consola para ver el detalle real.`
    : TILE_ERROR_MESSAGE;
}
...
export async function downloadVisibleArea(view, options = {}) {
  ...
  if (!getCurrentOriginAuthorization(location.origin).authorized) {
    throw new Error(UNAUTHORIZED_ORIGIN_MESSAGE);
  }
  ...
  for (const tile of tiles) {
    ...
    try {
      console.log("[offline-map] origin:", location.origin);
      console.log("[offline-map] tile url:", url);
