...
import { buildTileUrl, findCachedTileUrl, getOfflineTileSummary, getTransparentTileUrl } from "./offlineTileService.js";

function getArcGISBaseUrl() {
  if (appConfig.arcgisSdk?.useLocal) {
    return new URL(appConfig.arcgisSdk.localBasePath || "../../vendor/arcgis/@arcgis/core", import.meta.url).href.replace(/\/$/, "");
  }

  return (appConfig.arcgisSdk?.cdnBaseUrl || "https://js.arcgis.com/4.33/@arcgis/core").replace(/\/$/, "");
}

function getArcGISAssetsPath() {
  return `${getArcGISBaseUrl()}/assets`;
}

function ensureArcGISStylesheet() {
  const href = `${getArcGISAssetsPath()}/esri/themes/light/main.css`;
  const existing = document.querySelector('link[data-arcgis-theme="true"]');
...
async function ensureArcGISModules() {
  ...
  if (!arcgisState.loadPromise) {
    const arcgisBaseUrl = getArcGISBaseUrl();
    ensureArcGISStylesheet();
    arcgisState.loadPromise = Promise.all([
      import(`${arcgisBaseUrl}/config.js`),
      import(`${arcgisBaseUrl}/Map.js`),
      import(`${arcgisBaseUrl}/WebMap.js`),
      import(`${arcgisBaseUrl}/views/MapView.js`),
      import(`${arcgisBaseUrl}/Graphic.js`),
      import(`${arcgisBaseUrl}/Ground.js`),
      import(`${arcgisBaseUrl}/geometry/Point.js`),
      import(`${arcgisBaseUrl}/layers/ElevationLayer.js`),
      import(`${arcgisBaseUrl}/layers/GraphicsLayer.js`),
      import(`${arcgisBaseUrl}/layers/BaseTileLayer.js`),
      import(`${arcgisBaseUrl}/layers/TileLayer.js`),
      import(`${arcgisBaseUrl}/widgets/Search.js`),
      import(`${arcgisBaseUrl}/geometry/support/webMercatorUtils.js`),
    ]).then(([
      { default: esriConfig },
      { default: ArcGISMap },
      { default: WebMap },
...
function configureArcGISRuntime(esriConfig) {
  ...
  esriConfig.assetsPath = getArcGISAssetsPath();
  esriConfig.request.timeout = navigator.onLine ? 15000 : 3500;
}
...
function createOfflineTileLayer() {
  const { BaseTileLayer } = getArcGISModules() || {};
  const summary = getOfflineTileSummary();
  const manifest = summary.manifest;

  if (!BaseTileLayer || !summary.hasTiles || !manifest.layerUrl) {
    return null;
  }

  const OfflineTileLayer = BaseTileLayer.createSubclass({
    properties: {
      urlTemplate: manifest.urlTemplate || `${manifest.layerUrl}/tile/{z}/{y}/{x}`,
    },

    createTile(level, row, col) {
      const image = document.createElement("img");
      image.alt = "";
      image.width = 256;
      image.height = 256;
      image.decoding = "async";
      image.src = getTransparentTileUrl();

      findCachedTileUrl(level, row, col).then((cachedUrl) => {
        image.src = cachedUrl || getTransparentTileUrl();
      }).catch(() => {
        image.src = getTransparentTileUrl();
      });
      image.onerror = () => {
  ...
  return new OfflineTileLayer({
    id: "offline-tile-basemap",
    title: "Mapa offline descargado",
    copyright: "Tiles descargados para uso offline",
    tileInfo: manifest.tileInfo || undefined,
    fullExtent: manifest.fullExtent || undefined,
    spatialReference: manifest.spatialReference || { wkid: 102100 },
  });
}

async function buildMap({ offlineTiles = false } = {}) {
...
export async function initMap(container, { onCapture, onSelectCollar, offlineTiles = false } = {}) {
  if (!supportsWebGL2()) {
  ...
  configureArcGISRuntime(esriConfig);
  ...
  const map = await buildMap({ offlineTiles });
  ...
  if (!offlineTiles && appConfig.map.basemap) {
    try {
...
export async function generateTileUrls(minZoom = 10, maxZoom = 16) {
  ...
  return tilePackage.tiles.map((tile) => tile.url);
}

function getSerializableTileInfo(tileInfo) {
  if (!tileInfo) {
    return null;
  }

  if (typeof tileInfo.toJSON === "function") {
    return tileInfo.toJSON();
  }

  return {
    dpi: tileInfo.dpi,
    format: tileInfo.format,
    size: tileInfo.size || [tileInfo.rows || 256, tileInfo.cols || 256],
...
function getSerializableExtent(extent) {
  if (!extent) {
    return null;
  }

  return typeof extent.toJSON === "function" ? extent.toJSON() : extent;
}

async function getOfflineDownloadLayer(basemap) {
  const { TileLayer } = await ensureArcGISModules();
  let layer = basemap?.baseLayers?.find?.((candidate) => {
    const declaredClass = String(candidate?.declaredClass || "");
    return candidate?.url && candidate?.tileInfo && !declaredClass.includes("VectorTileLayer");
  });
  ...
  const offlineLayerUrl = String(appConfig.map.offlineTileLayerUrl || "").trim();
  if (!offlineLayerUrl || !TileLayer) {
    return null;
  }

  layer = new TileLayer({ url: offlineLayerUrl });
  await layer.load();
  return layer;
}

export async function generateVisibleTilePackage(minZoom = appConfig.map.offlineZoomMin || 10, maxZoom = appConfig.map.offlineZoomMax || 16) {
  ...
  try {
    ...
    const layer = await getOfflineDownloadLayer(basemap);
    if (!layer) {
      return { tiles: [] };
    }
      ...
      for (let row = minRow; row <= maxRow; row++) {
        for (let col = minCol; col <= maxCol; col++) {
          ...
          tiles.push({
            ...
            row: y,
            col: x,
            url: buildTileUrl(layerUrl, z, y, x),
          });
        }
      }
    }
    ...
    return {
      tiles,
      layerUrl,
      urlTemplate: `${layerUrl.replace(/\/$/, "")}/tile/{z}/{y}/{x}`,
      tileInfo: getSerializableTileInfo(tileInfo),
      spatialReference: tileInfo.spatialReference?.toJSON?.() || tileInfo.spatialReference || { wkid: 102100 },
      fullExtent: getSerializableExtent(layer.fullExtent),
      bounds: getSerializableExtent(view.extent),
      zoomMin: minZoom,
      zoomMax: maxZoom,
    };
  } catch {
