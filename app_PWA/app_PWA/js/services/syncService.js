...
function getBackendEndpoint(entityType) {
  return entityType === "proyecto"
    ? appConfig.sync.backend.endpoints.projects
    : appConfig.sync.backend.endpoints.collars;
}
...
async function pushToBackend(entityType, queueItem) {
  ...
  try {
    ...
    return {
      skipped: false,
      remoteId: result.globalId || result.global_id || result.id || "",
